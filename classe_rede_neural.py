...
class rede_neural():
    ...
    def forward_propagation_GPU(self, x):
        ...
        for l in range(0, self.L):
            for j in range(0, self.m[l + 1]):
                self.l[l].v[j] = __compute_neuron_GPU__(self.l[l].w[j], input)
                # self.l[l].v[j] = np.matmul(np.transpose(self.l[l].w[j]), input)
                self.l[l].y[j] = activation_func_GPU(self.a[l], self.b[l], self.l[l].v[j])
...
def calculate_fitness(test_dataset, rede, num_classes, bias_classes=None, name=0, processor='CPU'):
